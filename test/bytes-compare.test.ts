...
describe('bytes/compare', () => {
	...
	test('timingSafeEqual returns false for differing lengths', () => {
		expect(
			timingSafeEqual(new Uint8Array([1, 2, 3]), new Uint8Array([1, 2]))
		).toBe(false);
	});
	...
	test('bytesEqual matches timingSafeEqual on positive cases', () => {
		...
		expect(
			bytesEqual(new Uint8Array([7, 8, 9]), new Uint8Array([7, 8, 9]))
		).toBe(true);
	});

	test('bytesEqual rejects mismatches and length differences', () => {
		expect(
			bytesEqual(new Uint8Array([7, 8, 9]), new Uint8Array([7, 0, 9]))
		).toBe(false);
		expect(
			bytesEqual(new Uint8Array([7, 8]), new Uint8Array([7, 8, 9]))
		).toBe(false);
	});

	test('concatBytes joins zero, one, and many arrays', () => {
		expect(Array.from(concatBytes())).toEqual([]);
		expect(Array.from(concatBytes(new Uint8Array([1, 2, 3])))).toEqual([
			1, 2, 3,
		]);
		expect(
			Array.from(
